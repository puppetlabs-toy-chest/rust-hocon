...
named!(object < Vec<(&str,JsonValue)> >, delimited!(char!('{'), separated_list!(char!(','),keypair), char!('}')));
...
named!(object_map < JsonValue >,
       delimited!(char!('{'),
                  map!(separated_list!(char!(','),keypair), |x| { JsonValue::Object(pairs_to_map(x)) } ),
                  char!('}')));

named!(json_value<JsonValue>, alt!(json_null | json_float | json_int  | quoted_str_val | json_boolean | object_map | json_array));

named!(json_array < JsonValue >,
       delimited!(char!('['),
                  map!(separated_list!(char!(','), json_value), JsonValue::Vec  ),
                  char!(']')));
...
fn check_quoted_str(){
    ...
    assert_eq!(quoted_str_val(&b"\"foo\""[..]), IResult::Done(empty,JsonValue::Str("foo")));
    assert_eq!(quoted_str(&b"\"f\\\"o\\\"o\""[..]), IResult::Done(empty,"f\\\"o\\\"o"));
    assert_eq!(quoted_str_val(&b"\"f\\\"o\\\"o\""[..]), IResult::Done(empty,JsonValue::Str("f\\\"o\\\"o")));
}


#[test]
fn check_int(){
    assert_eq!(json_int(&b"123"[..]), IResult::Done(&b""[..],JsonValue::Int(123)));
}


#[test]
fn check_float(){
    assert_eq!(json_float(&b"123.123"[..]), IResult::Done(&b""[..],JsonValue::Float(123.123)));
}
...
fn check_pairs_to_map(){
    ...
    assert_eq!(res,pairs_to_map(vec![("a",JsonValue::Int(1)),("b",JsonValue::Int(2)), ("c",JsonValue::Int(3))]));
}
